...
def _claims(answer: str) -> list[str]:
    """Return material, human-readable claim lines for citation attachment scoring.

    This is deliberately conservative: headings, separators and tiny fragments are
    ignored. It measures whether factual answer claims carry valid evidence IDs;
    it does not pretend to be an entailment model.
    """
    out: list[str] = []
    for raw in (answer or "").splitlines():
        ...
        if len(re.sub(r"\[[^\]]+\]", "", line).strip()) < 20:
            continue
        # Treat each substantial bullet/table row as one reviewable claim unit.
        out.append(line)
    return out
