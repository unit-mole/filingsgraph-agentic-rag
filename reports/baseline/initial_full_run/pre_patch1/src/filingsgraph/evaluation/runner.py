...
def evaluate_retriever(questions:list[dict],search_fn,out_path:str|Path)->dict:
    ...
    summary={k:(sum(r[k] for r in rows)/len(rows) if rows else None) for k in ['r5','r10','p5','hit10','mrr','ndcg10','latency_ms']}
    payload={"summary":summary,"rows":rows}; p=Path(out_path);p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(payload,indent=2),encoding='utf-8');return payload
