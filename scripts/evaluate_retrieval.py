...
def main():
    ...
    def filters_for(question: str) -> dict | None:
        return infer_query_filters(question, tickers) or None
    ...
    systems = [
        ...
        ("hybrid_filtered", lambda q: hybrid.search(q, dense_top_k=30, sparse_top_k=30, fusion_top_k=10, filters=filters_for(q))),
    ]
    if not args.skip_reranker and not args.smoke:
        ...
        systems.append(("hybrid_reranked", lambda q: reranker.blended_rerank(
            q,
            hybrid.search(q, dense_top_k=30, sparse_top_k=30, fusion_top_k=30, filters=filters_for(q)),
            top_k=10,
    ...
    for name, fn in systems:
        ...
        print("  by_category", json.dumps(r.get("by_category", {}), indent=2))
