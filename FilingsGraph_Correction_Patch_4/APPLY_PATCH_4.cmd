...
if exist "%CD%\src\filingsgraph" (
  ...
  echo ERROR: Run this from the FilingsGraph project root, or keep the Patch 4 folder directly under the project root.
  exit /b 1
)

pushd "%ROOT%"

echo [1/7] Verifying the frozen 17-file core before Patch 4...
python -m scripts.check_v6_freeze
if errorlevel 1 (popd & exit /b 1)

echo [2/7] Backing up pre-Patch-4 specialized files...
set "BACKUP=%ROOT%\reports\baseline\v6_specialized_gold_baseline\pre_patch4_code"
...
if exist "src\filingsgraph\llm\prompts.py" copy /Y "src\filingsgraph\llm\prompts.py" "%BACKUP%\llm_prompts.py" >nul
...
if exist "src\filingsgraph\verification\citations.py" copy /Y "src\filingsgraph\verification\citations.py" "%BACKUP%\citations.py" >nul
if exist "scripts\evaluate_grounding.py" copy /Y "scripts\evaluate_grounding.py" "%BACKUP%\evaluate_grounding.py" >nul
if exist "scripts\evaluate_temporal.py" copy /Y "scripts\evaluate_temporal.py" "%BACKUP%\evaluate_temporal.py" >nul
if exist "scripts\evaluate_graph.py" copy /Y "scripts\evaluate_graph.py" "%BACKUP%\evaluate_graph.py" >nul

echo [3/7] Applying Patch 4 specialized-layer files...
...
echo Next: rebuild only the graph, regenerate temporal predictions, run Patch-4 diagnostic, then run live grounding.
endlocal
