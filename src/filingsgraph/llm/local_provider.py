...
class LocalTransformersProvider(ModelProvider):
    def __init__(self,model_name:str|None=None,device:str|None=None):
        ...
        self.model=AutoModelForCausalLM.from_pretrained(self.model_name,dtype=dtype,device_map='auto' if self.device.startswith('cuda') else None)
    def generate(self,messages:list[dict],max_new_tokens:int=1400,temperature:float=0.2,**kwargs)->str:
        ...
        return strip_reasoning(self.tokenizer.decode(out[0][n:],skip_special_tokens=True))
...
class OpenAICompatibleLocalProvider(ModelProvider):
    ...
    def generate(self,messages:list[dict],max_new_tokens:int=1400,temperature:float=0.2,**kwargs)->str:
        ...
        r.raise_for_status(); return strip_reasoning(r.json()['choices'][0]['message']['content'])
